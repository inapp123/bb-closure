/*
 * Minimal UTF-8 helpers and wcwidth (East Asian Width) for BB+AA-lib.
 * wcwidth tables derived from Markus Kuhn's implementation.
 */
...
const char *
utf8_next(const char *s, uint32_t *cp)
{
    unsigned char c;
...
static int
in_interval(uint32_t ucs, const struct interval *table, int count)
{
    int min = 0;
    int max = count - 1;
    int mid;
...
/* Interval tables for zero-width and double-width characters. */
static const struct interval combining[] = {
    ...
    {0x0A01, 0x0A03}, {0x0A3C, 0x0A3C}, {0x0A41, 0x0A42},
    {0x0A47, 0x0A48}, {0x0A4B, 0x0A4D}, {0x0A51, 0x0A51},
    {0x0A70, 0x0A71}, {0x0A75, 0x0A75}, {0x0A81, 0x0A83},
    {0x0ABC, 0x0ABC}, {0x0AC1, 0x0AC5}, {0x0AC7, 0x0AC8},
    {0x0ACD, 0x0ACD}, {0x0AE2, 0x0AE3}, {0x0B01, 0x0B03},
    ...
    {0x0C00, 0x0C03}, {0x0C3E, 0x0C40}, {0x0C46, 0x0C48},
    {0x0C4A, 0x0C4D}, {0x0C55, 0x0C56}, {0x0C62, 0x0C63},
    {0x0C81, 0x0C83}, {0x0CBC, 0x0CBC}, {0x0CBF, 0x0CBF},
    {0x0CC6, 0x0CC6}, {0x0CCC, 0x0CCD}, {0x0CE2, 0x0CE3},
    {0x0D01, 0x0D03}, {0x0D41, 0x0D44}, {0x0D4D, 0x0D4D},
    ...
    {0x180B, 0x180D}, {0x18A9, 0x18A9}, {0x1920, 0x192B},
    {0x1930, 0x193B}, {0x19B0, 0x19C0}, {0x19C8, 0x19C9},
    {0x1A17, 0x1A1B}, {0x1A55, 0x1A5E}, {0x1A60, 0x1A7C},
    {0x1A7F, 0x1A7F}, {0x1AB0, 0x1ABE}, {0x1B00, 0x1B03},
    {0x1B34, 0x1B34}, {0x1B36, 0x1B3A}, {0x1B3C, 0x1B3C},
    {0x1B42, 0x1B42}, {0x1B6B, 0x1B73}, {0x1BA0, 0x1BA0},
    {0x1BAE, 0x1BAF}, {0x1BCD, 0x1BCD}, {0x1BE6, 0x1BE6},
    ...
    {0xAB5B, 0xAB5B}, {0xFB1E, 0xFB1E}, {0xFE00, 0xFE0F},
...
static const struct interval wide[] = {
    ...
    {0xFF01, 0xFF60}, {0xFFE0, 0xFFE6}, {0x1F300, 0x1F64F},
    {0x1F900, 0x1F9FF}
};

int
utf8_column_width(uint32_t ucs)
{
    if (ucs == 0)
        return 0;
    if (ucs == AA_GLYPH_WIDE_PAD)
        return 0;
    if (ucs < 32 || (ucs >= 0x7F && ucs < 0xA0))
        return -1;
    if (in_interval(ucs, combining,
                    (int) (sizeof(combining) / sizeof(combining[0]))))
        return 0;
    if (in_interval(ucs, wide, (int) (sizeof(wide) / sizeof(wide[0]))))
        return 2;
    return 1;
}
