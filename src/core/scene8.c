...
static void mydraw(void)
{
    ...
    if (mul2 < 0)
	mul1 -= mul2, mul2 = 0;
    cx = IN(xs, xe) + sin(STATE1 / 380000.0) * 18;
    cy = IN(ys, ye) + cos(STATE1 / 420000.0) * 18;
    sx = IN(sxs, sxe) + sin(STATE1 / 460000.0) * 25;
    sy = IN(sys, sye) + cos(STATE1 / 460000.0) * 25;
...
void scene8()
{
    ...
    re = 100;
    cs = 100, ce = 100;
    /* 470x470 emblem: (500,500) = image center, sx=sy keeps square crop */
    xs = xe = 500;
    ys = ye = 500;
    sxs = sys = sxe = sye = 70;
